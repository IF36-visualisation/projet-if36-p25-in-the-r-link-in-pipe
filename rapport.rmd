...
Nous avons commencé par nous pencher sur le temps que prennent les joueurs à finir les jeux, c'est-à-dire notre questionnement principal au début du projet.
...
Ces variations peuvent s'expliquer de différentes manières. Cependant, la lisibilité de ce graphique me semblant encore assez complexe, j'ai décidé de l'adapter pour essayer d'y voir un peu plus clair d'abord, notamment en prenant une **échelle logarithmique**, et un autre type de graphique : le **Violin plot**.
...
Nous nous sommes ensuite penchés sur le temps moyen passé par les joueurs sur chaque jeu.
...
Le graphe précédent ayant soulevé la question des notes données aux jeux par les joueurs, nous avons ensuite étudié plus concrètement les avis que reçoivent les jeux.

## Y'a t il une relation entre le prix d'un jeu, sa note et le nombre de personnes qui le possèdent ?
...
*Soda Drinker Pro* est un cas à part, étant intentionnellement mal conçu. Cela explique le score bas de la presse, qui essait de présenter le jeu de manière objective, mais aussi le score plus élevé de la part des joueurs, qui prennent le jeu avec humour et apprécient son ironie. *Super Seducer* est un autre cas à part. La presse lui reproche d'être sexiste et d'objectifier les femmes, mais on peut supposer qu'une partie des joueurs l'ayant acheté étaient déjà consients de ces problèmes et ne l'auraient pas acheté si cela les dérangeait, ce qui explique le score plus élevé.  
...
Nous avons pu voir que le genre d'un jeu influençait grandement sa durée de complétion, mais que ce n'était pas le cas de la plateforme sur lequel il est joué. Nous avons aussi remarqué qu'il y a beaucoup plus de jeux disponibles sur Windows que sur Mac ou Linux.

Nous avons constaté que les genres de jeux multijoueurs et le jeux d'aventure ont souvent un temps de jeu important, et que les jeux les plus vendus ont souvent un temps de jeu moyen peu élevé. Nous avons aussi pu voir que les avis donnés par les joueurs sur Steam n'influençaient que très peu le temps passé par les joueurs sur le jeu.

Enfin, nous avons pu remarquer que le nombre de possesseurs influençait le pourcentage d'avis positifs d'un jeu. Nous avons aussi pu noter que les jeux de grandes franchises ont souvent de très bonnes critiques, tandis que les jeux peu connus reçoivent des avis plus variables.

## Difficultés rencontrées

Lors de la rédaction de la proposition de projet, nous nous étions apperçus que le format json dans lequel était formaté le dataset How Long To Beat n'était pas très pratique. Cependant, nous ne nous attendions pas à avoir autant de difficulté à l'exploiter. Une fois que nous avons trouvé les bons outils pour transformer ce 

Nous avons rencontré quelques difficultés entre nos différents ordinateurs. Certains ne comprenaient pas les caractères chinois dans certains titres, tandis que d'autres réagissaient mal à la fonction de filtre créée pour pallier à ce problème.

Notre groupe n'a pas été extrêmement bien organisé, et nous avons dû réaliser certaines parties du travail au dernier moment. Si le projet était à refaire, nous nous y prendrions plus tôt, et nous communiquerions peut-être plus entre nous. Nous n'avons discuté que par messages pour la majorité du projet, et cela nous a peut-être porté préjudice.

## Pistes d'approfondissement

Il serait possible d'explorer l'influence d'autres variables sur le temps de jeu ou la durée de complétion.

On pourrait aussi explorer plus en détail la différence entre les conséquences des avis de la presse et de ceux des joueurs.
...
- **Rédaction du Rapport final** : Lucile - Tifenn
  - **Rédaction de la conclusion** : Lucile
  - **Rédaction de l'annexe** : Tifenn
  - **Réorganisation et harmonization du contenu** : Lucile
- **PowerPoint** : Lucile
- **Shiny** : Virgile
- **Tableau** : Ilan
...
- "Y'a t il une relation entre le prix d'un jeu, sa note et le nombre de personnes qui le possèdent ?" :  Lucile (scatter plot) & Virgile (heatmap)
- "Les avis des joueurs sur les jeux sont-ils différents de ceux de la presse spécialisée ?" : Lucile
